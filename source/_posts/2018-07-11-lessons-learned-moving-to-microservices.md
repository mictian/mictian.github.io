...
# Lessons learned migrating to Microservices
...
Before we start talking about what have been my experience so far, lets first recap the things that we must "suffer" or the things that we must understand in order to move to micro-services.
...
The first question that you must ask to yourself is **why do we want to move to a microservices architecture?**.   
If in your answer you are not describing a business problem, then your are destined to failure (perhaps not so dramatically). It seems that in IT companies, engineers tend to forget the reason why companies exists: To solve a customer problem using software as a means to an end.

Although this is obvious and at first nonessential, the main idea is to keep business needs and engineering as one aligned team in which all members are trying to solve the same problem.
        
To emphasize the difference between technical needs and business needs, lets just list some common examples:
...
* *Move fast*: "We need to rebuild our current monolith so we will be able to generate better tracing of our app".
Again, not backed by any direct business value. Do you know for sure that adding better tracing or being prepared to troubleshoot an error in production is valuable from the business point of view? Without a doubt, from the technical point of view this statement is trying to improve the infrastructure, but without business alignment you don't know if the company is planning to sell that part of the business.


* *Maintenance cost*: Believe it or not, ethical or not, there are companies that depend on high maintenance costs to subsist, so improving the code base can actually be harmful (lets leave the ethical discussion for another occation - there are plenty of reasons why I am not a director/manager).

Please, do not take this as a way to express or describe a problem, but rather as a recommendation to see if your journey into microservices has the right reasons to start. Otherwise, in my experience, you will face uncountable discussions between management and engineering.
I, as an engineer, totally understand the value of having a better code; from less maintenance cost to greater speed to develeop new features. But make sure that business guys in your organization see the value as well.
...
(2) My second lesson was that **whoever is responsible to start the microservices transition should try their best to not over-engineer the problem at hand.**
...
I could continue writing sentences on how teams try to justify their overwork and over-engineering efforts, but based on my experience, teams end up chasing ghosts; solving no one problems and refactoring the code over and over again without really adding value.
So, my recommendation is, without forgetting what you will need to do next, keep the focus. Develop the framework later, once that you understand what are the pieces of it.
...
A final observation in this lesson is to not forget how to develop software. Although it sounds stupid, but it is quite interesting to see companies that start planning microservices infrastructure going layer by layer.
First define the database schema, then the business logic, and once that is all in place plan how and when to do the UI.
...
Assume that the process will take time and pushing it won't help.
...
In the same way that in a call center it's not possible to use waterfall (how would you predict how many calls you will receive the first day?) A company that moves to microservices needs to adhere to agile principles (It does not need clarification that this is my vision).
...
If in your company you're a "control freak", micro-managing people and your organization chart is higher that wider... Well, my experience determines that you will face serious issues.

A subtle and sometimes unspoken point about how microservices scales, is that they scale by scaling the teams behind them. And by this I mean horizontally, making the hierarchical pyramid of your company higher, believe me, won't scale.

Another important phenomenon that occurs from time to time, is that stakeholders start to get exited and push the dev team. Remember that it must be a PO and he/she must have a clear role in this process.
...
Clearly, at first, this sounds as a good idea; someone with more experience must know what the common problems are and the best practices, so in this way the company will save time and money.

If you understand, believe and practice agile principles, sooner than later you will see how this tends to be a bad idea.

Leaders are not imposed by managers/directors,  specially technical ones. Leader are choosen by the teams.
Generally a team member is recognized as such. Someone that the team trusts and knows.
...
Trust in your team, after all they developed the software you are using.
...
In this section I won't present any technical pattern neither any revolutionary new technic, but just a few basic remarks that I have observed while helping other companies in this transition.
...
Just a simple note in case that you reader are not a technical guy, it won't be possible to use a different DB engine without applying at least some minor changes to the Business Logic Layer, the fact the we discuss about data first is just a matter on where we are putting the focus.

(7) **Unless that you are working on a rather new company, don't think that your DB schema is well organized.**
...
Try to not build a data synchronization framework. I don't know why, lots of engineers ended up needing one. Please evaluate if it is really needed, and if that is the case (in many cases is not) try not to build a fancy one from scratch.
...
The main risk when extracting pieces of logic is that you break one strange, undocumented and forgotten use case. So for this reason try to go step by step, using all your testing arsenal and validate every single use case migrated with your PO.
...
Although in the vast majority of the cases this is the selected strategy, it is not necessarily the easiest to implement.

(9) **The way of working while doing maintenance generally is not the same as when developing new software.**
We all agree that the focus is to add business value, but in incremental steps that are achievable. I would strongly recommend to double check if this is the best approach.
...
I'm proposing these silly solutions because in this case engineers tend to go wild, creating reusable components, using cloud providers, move to dockers, etc. etc. Those not necessarily are bad solutions, if the organization is aligned and aiming to go in that direction.
...
* Whatever is your answer, you will need to touch the code base of your monolithic. Your current application won't start interacting with a new service/s automagically. (Yes, this assumption happens and generates lot of discussions between directors and developers).
